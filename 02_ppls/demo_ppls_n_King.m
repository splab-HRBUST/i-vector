...
for iTers = 1 : 15
    if iTers == 1
        [patameters_PPLS] = ppls_n_em(m_dev,y_dev,num);
    else
        [patameters_PPLS] = ppls_n_em(m_dev,y_dev,num,patameters_PPLS);
    end
...
[eer_PPLS400_CDS(iTers),~,~,~,dcf_king_PPLS400_CDS(iTers)]=compute_eer(scores_PPLS_CDS,answer_eva,false);
...
[eer_PPLS400_PLDA200(iTers),~,~,~,dcf_king_PPLS400_PLDA200(iTers)]=compute_eer(scores_PPLS_PLDA,answer_eva,false);

fprintf('===== Summaray ======= Baseline =====%d=========\n',iTers);
fprintf('+-------------------------------------------+\n');
fprintf('|    Method     |    EER(%%)   | Min DCF_vox |\n');
fprintf('+---------------+-------------+-------------+\n');
fprintf('|PPLS(%d)+CDS     |    %2.2f    |   %2.4f    |\n', num.IVdim,         eer_PPLS400_CDS(iTers),    dcf_king_PPLS400_CDS(iTers));
fprintf('|PPLS(%d)+PLDA(%d)|    %2.2f    |   %2.4f    |\n', num.IVdim,num.Zdim,eer_PPLS400_PLDA200(iTers),dcf_king_PPLS400_PLDA200(iTers));
fprintf('+---------------+-------------+-------------+\n');
