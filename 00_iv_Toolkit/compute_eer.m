...
if ( nargout == 5 ),
    ...
    Cdet  = Cmiss * FNR * P_tgt + Cfa * FPR * ( 1 - P_tgt);
%     Cdefault = min(Cmiss * P_tgt, Cfa * ( 1 - P_tgt));
    dcf_king = 100 * min(Cdet); % note this is not percent
end
...
% xtick = [0.03, 0.04, 0.05, 0.06, 0.07, 0.075, 0.085, 0.1, 0.126, 0.15, 0.17, 0.19, 0.23, 0.25]; %for the VoxCeleb
xticklabel = num2str(xtick * 100, '%g\n');
xticklabel = textscan(xticklabel, '%s'); xticklabel = xticklabel{1};
set (gca, 'xtick', icdf(xtick));
set (gca, 'xticklabel', xticklabel);
% xlim(icdf([0.00051 0.5])); % default
xlim(icdf([0.005 0.4])); % for the VoxCeleb
% xlim(icdf([0.05 0.25])); % for the VoxCeleb
...
% ylim(icdf([0.00051 0.5]));  % default
ylim(icdf([0.005 0.4]));  % for the VoxCeleb
% ylim(icdf([0.05 0.25])); % for the VoxCeleb
