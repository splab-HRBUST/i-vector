...
num.Zdim = 200; % dimension of latent vector for PLDAs
% 
%  
num.nIters = 1;
...
for iTers = 6 : 20
    if iTers == 1
        patameters_PPCA_baseline = ppca_em(M_dev, ubm, num);
    else
        patameters_PPCA_baseline = ppca_em(M_dev, ubm, num, patameters_PPCA_baseline);
    end
        
%% prepare for evaluation
Ex_dev = pinv(patameters_PPCA_baseline.L)*patameters_PPCA_baseline.B*(M_dev-patameters_PPCA_baseline.m);
Ex_eva = pinv(patameters_PPCA_baseline.L)*patameters_PPCA_baseline.B*(M_eva-patameters_PPCA_baseline.m);
% 
answer_eva=[];
target1_IVs=[];
target2_IVs=[];
...
[eer_baseline_PPCA400_CDS(iTers),~,~,dcf_vox_baseline_PPCA400_CDS(iTers)]=compute_eer(scores_CDS,answer_eva,false);
...
[eer_baseline_PPCA400_PLDA200(iTers),~,~,dcf_vox_Baseline_PPCA400_PLDA200(iTers)]=compute_eer(scores_PLDA,answer_eva,false);
...
% 
fprintf('===== Summaray ========== PPCA =====%d===========\n',iTers);
fprintf('+-------------------------------------------+\n');
fprintf('|       Method      |    EER(%%)   | Min DCF_vox |\n');
fprintf('+-------------------+-------------+-------------+\n');
fprintf('|  PPCA(%d)+CDS    |    %2.2f    |   %2.4f    |\n', num.IVdim,        eer_baseline_PPCA400_CDS(iTers),    dcf_vox_baseline_PPCA400_CDS(iTers));
fprintf('|PPCA(%d)+PLDA(%d)|    %2.2f    |   %2.4f    |\n', num.IVdim,num.Zdim,eer_baseline_PPCA400_PLDA200(iTers),dcf_vox_Baseline_PPCA400_PLDA200(iTers));
fprintf('+---------------+-------------+-------------+\n');
...
% +-------------------+-------------+-------------+
% |  PPCA(400)+CDS    |    15.89    |   0.8333    |
% |PPCA(400)+PLDA(200)|     8.03    |   0.6685    |
% +-------------------+-------------+-------------+
