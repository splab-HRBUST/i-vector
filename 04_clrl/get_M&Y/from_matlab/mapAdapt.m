...
%
% MAP使用dataList中的功能从UBM ubmFilename中适配特定于说话人的GMM gmmFilename。
%可以通过tau指定地图相关系数。支持所有GMM超参数的自适应。
%
% Inputs:
%   - dataList    : ASCII file containing adaptation feature file name(s)  数据列表：包含自适应功能文件名的ASCII文件
%                   or a cell array containing feature(s). Feature files   或包含特征的单元阵列。要素文件
%					must be in uncompressed HTK format.                    必须是未压缩的HTK格式
