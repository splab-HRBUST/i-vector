...
%
%使用EM算法训练给定训练数据的PLDA模型。扬声器标签在spk_labs中给出。nphi确定特征语音子空间的维数，niter指定EM迭代次数。
%
...
% 对于i向量，代码计算因子加载矩阵Phi（又称本征语音子空间）的最大似然估计（MLE）。
% 这里，x是i向量，m是训练i向量的平均值，y~N（0，i）是潜在因子的向量。
% 剩余噪声项e解释了未通过潜在变量捕获的可变性
%
% Inputs:
%   data            : input data matrix, one observation per column   输入数据矩阵，每列一个观测值
%   spk_labs        : class labels for observations in data matrix    数据矩阵中观测值的类标签
%   nphi            : dimensionality of the Eigenvoice subspace       本征语音子空间的维数
