...
for iTers = 1 : 15
    if iTers == 1
        parameters_PPCA = ppca_em(M_dev, ubm, num);
    else
        parameters_PPCA = ppca_em(M_dev, ubm, num, parameters_PPCA);
    end
        
    %% prepare for evaluation
    IV_dev = pinv(parameters_PPCA.L)*parameters_PPCA.B*bsxfun(@minus, M_dev, parameters_PPCA.m);
    IV_eva = pinv(parameters_PPCA.L)*parameters_PPCA.B*bsxfun(@minus, M_eva, parameters_PPCA.m);
    ...
    [eer_PPCA400_CDS(iTers),~,dcf_vox_PPCA400_CDS(iTers)]=compute_eer(scores_CDS,answer_eva,false);

    fprintf('================= LDA+cosine ====================\n');
    scores_LDA_CDS = 1 - pdist2(finalTarget2',finalTarget1','cosine');
    scores_LDA_CDS = diag(scores_LDA_CDS);
    [eer_PPCA400_LDA200_CDS(iTers),~,dcf_vox_PPCA400_LDA200_CDS(iTers)]=compute_eer(scores_LDA_CDS,answer_eva,false);
    ...
    [eer_PPCA400_PLDA200(iTers),~,dcf_vox_PPCA400_PLDA200(iTers)]=compute_eer(scores_PLDA,answer_eva,false);
    ...
    [eer_PPCA400_LDA200_PLDA200(iTers),~,dcf_vox_PPCA400_LDA200_PLDA200(iTers)]=compute_eer(scores_LDA_PLDA,answer_eva,false);

    fprintf('===== Summaray ======= Baseline =====%d=========\n',iTers);
    fprintf('+-------------------------------------------+\n');
    fprintf('|    Method     |    EER(%%)   | Min DCF_vox |\n');
    fprintf('+---------------+-------------+-------------+\n');
    fprintf('|      CDS      |    %2.2f    |   %2.4f    |\n', eer_PPCA400_CDS(iTers),dcf_vox_PPCA400_CDS(iTers));
    fprintf('|    LDA+CDS    |    %2.2f    |   %2.4f    |\n', eer_PPCA400_LDA200_CDS(iTers),dcf_vox_PPCA400_LDA200_CDS(iTers));
    fprintf('|PPCA(%d)+PLDA(%d)|    %2.2f    |   %2.4f    |\n', num.IVdim,num.Zdim,eer_PPCA400_PLDA200(iTers),dcf_vox_PPCA400_PLDA200(iTers));
    fprintf('|PPCA(%d)+LDA+PLDA(%d)|    %2.2f    |   %2.4f    |\n', num.IVdim,num.Zdim,eer_PPCA400_LDA200_PLDA200(iTers),dcf_vox_PPCA400_LDA200_PLDA200(iTers));
    fprintf('+---------------+-------------+-------------+\n');
