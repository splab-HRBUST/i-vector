...
for iTers = 1 : 100
    if iTers == 1
        parameters_BPCA = bpca_em(M_dev, num);
    else
        parameters_BPCA = bpca_em(M_dev, num, parameters_BPCA);
    end
        
    %% prepare for evaluation
    IV_dev = pinv(parameters_BPCA.L)*parameters_BPCA.B*bsxfun(@minus, M_dev, parameters_BPCA.m);
    IV_eva = pinv(parameters_BPCA.L)*parameters_BPCA.B*bsxfun(@minus, M_eva, parameters_BPCA.m);
    ...
    [eer_BPCA400_CDS(iTers),~,dcf_vox_BPCA400_CDS(iTers)]=compute_eer(scores_CDS,answer_eva,false);

    fprintf('================= LDA+cosine ====================\n');
    scores_LDA_CDS = 1 - pdist2(finalTarget2',finalTarget1','cosine');
    scores_LDA_CDS = diag(scores_LDA_CDS);
    [eer_BPCA400_LDA200_CDS(iTers),~,dcf_vox_BPCA400_LDA200_CDS(iTers)]=compute_eer(scores_LDA_CDS,answer_eva,false);
    ...
    [eer_BPCA400_PLDA200(iTers),~,dcf_vox_BPCA400_PLDA200(iTers)]=compute_eer(scores_PLDA,answer_eva,false);
    ...
    [eer_BPCA400_LDA200_PLDA200(iTers),~,dcf_vox_BPCA400_LDA200_PLDA200(iTers)]=compute_eer(scores_LDA_PLDA,answer_eva,false);

    fprintf('===== Summaray ======= Baseline =====%d=========\n',iTers);
    fprintf('+-------------------------------------------+\n');
    fprintf('|    Method     |    EER(%%)   | Min DCF_vox |\n');
    fprintf('+---------------+-------------+-------------+\n');
    fprintf('|      CDS      |    %2.2f    |   %2.4f    |\n', eer_BPCA400_CDS(iTers),dcf_vox_BPCA400_CDS(iTers));
    fprintf('|    LDA+CDS    |    %2.2f    |   %2.4f    |\n', eer_BPCA400_LDA200_CDS(iTers),dcf_vox_BPCA400_LDA200_CDS(iTers));
    fprintf('|BPCA(%d)+PLDA(%d)|    %2.2f    |   %2.4f    |\n', num.IVdim,num.Zdim,eer_BPCA400_PLDA200(iTers),dcf_vox_BPCA400_PLDA200(iTers));
    fprintf('|BPCA(%d)+LDA+PLDA(%d)|    %2.2f    |   %2.4f    |\n', num.IVdim,num.Zdim,eer_BPCA400_LDA200_PLDA200(iTers),dcf_vox_BPCA400_LDA200_PLDA200(iTers));
    fprintf('+---------------+-------------+-------------+\n');
